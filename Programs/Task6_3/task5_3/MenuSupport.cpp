...
void MenuSupport::runSecondMainMenu()
{
	...
	while (!canExit)
	{
		...
		switch (_userChoise)
		{
			...
			case kExitProgramSecondMainMenuTypes:		// почати ТУТ!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			{
...
void MenuSupport::showShoppingStoreList() const
{
	...
	{
		char* storeName;
		storeName = _shoppingStores->getStoreAtIndex(i)->getStoreName();
		printf("        %i. %s\n", i + 1, storeName);
	}
}
...
bool MenuSupport::isEnableMainMenuItem(int mainMenuItemType) const
{
	...
	switch (mainMenuItemType)
	{
		...
		case kAddStoreSecondMainMenuTypes:
		{
			result = _shoppingStores->getStoreCount() < kMaxStoreCount; // ?прибрати перевірку?
			break;
		}
		default:
			break;
...
char* MenuSupport::inputProductName()
{
